...
export const CreateProductModal = ({ closeModal, handleAddNewProduct, categories }: { categories: Category[], closeModal: () => void, handleAddNewProduct: (value: Product) => void }) => {
    ...
    return (
        <Modal title="Create Product" closeModal={closeModal}>
            ...
            <form className="flex flex-col gap-4 text-gray-950 justify-between h-full" onSubmit={e => { e.preventDefault(); handleCreateProduct() }}>
                <div className="flex flex-col gap-4 w-full">
                    ...
                    <label className="flex flex-col">
                        ...
                        {categories && categories.length > 0 ?
                            <select className="rounded bg-gray-200 py-2 px-4 w-full h-full" value={category?._id ?? ""} onChange={e => handleChangeSelectCategory(e)} required>
                ...
                <div className="flex justify-end gap-4 sm:flex-row flex-col mt-4">
                    <button type="button" className="text-gray-950 bg-gray-300 p-2 rounded hover:bg-gray-200 cursor-pointer transition" onClick={closeModal}>
                        Cancel
                    </button>
                    ...
                    <button type="submit" className="bg-green-800 text-green-50 p-2 rounded hover:bg-green-700 cursor-pointer inline-flex gap-2 items-center transition" disabled={isLoading}>
                        {isLoading &&
                            <div className="min-w-4 min-h-4 animate-spin rounded-full
